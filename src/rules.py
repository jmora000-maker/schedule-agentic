...
from src.models import RiskFinding, RiskSignal, Issue, TaskUpdate, ScheduleTask
...
class RuleEngine:
    ...
    def _get_milestone_for_task(self, task: ScheduleTask) -> Optional[str]:
        # Search for milestones reachable from this task
        visited = set()
        queue = [task.task_uid]
        ...
        while queue:
            ...
            successors = self.graph.get_successors(current_uid)
            for node in successors:
                if node.get("type") == "task":
                    task_uid_str = node["id"].replace("task-", "")
                    task_node = self.tasks_by_uid.get(int(task_uid_str))
                    
                    if task_node:
                        if task_node.is_milestone:
                            # Use task_uid as milestone_id if it's a milestone task
                            return str(task_node.task_uid)
                        
                        queue.append(task_node.task_uid)
        return None
        ...
        for task in self.context.tasks:
            if task.predecessor_uids:
                ...
                if delayed_preds and (task_is_slipped or task_is_critical):
                    ...
                    milestone_id = self._get_milestone_for_task(task)
                    findings.append(self._make_finding(
                        self._finding_id(rule_name, task=task, milestone_id=milestone_id),
                        rule_name, "schedule_delay", severity, confidence,
                        f"Task '{task.name}' is impacted by delayed predecessor(s).",
                        evidence,
                        task=task, milestone_id=milestone_id,
                        metadata=self._build_metadata(task=task, milestone_id=milestone_id)
                    ))
        return findings
